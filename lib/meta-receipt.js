...
/**
 * Generates a Meta-style transaction ID: two 17-digit numbers joined by a hyphen.
 * Real example: "26907590175595082-26986223884398370".
 * Used as a fallback when no transaction ID is provided in config.
 */
function generateMetaTxnId() {
...
function drawMetadata(doc, y, config, endDate, transactionId) {
  let leftY = y;
  ...
  drawLabelValue(doc, "Transaction ID", transactionId, leftY);
  leftY += 32;
...
export async function generateMetaReceipt({
  ...
  // Resolve the transaction ID up front so the same value appears on the PDF
  // body AND in the filename. Falls back to an auto-generated Meta-style ID
  // (two 17-digit numbers joined by hyphen).
  const transactionId = config.receiptTransactionId || generateMetaTxnId();
  ...
  const { finalY: leftEndY, rightPaidY } = drawMetadata(doc, y, config, endDate, transactionId);
  ...
  // === Trigger download ===
  // Match Meta's actual filename format:
  //   2026-04-01T16-53 Transaction #26907590175595082-26986223884398370.pdf
  // Components: <endDate>T<HH-mm> Transaction #<transactionId>.pdf
  // (space before "Transaction", "#" before the ID — same ID as printed on the PDF)
  const now = new Date();
  const hh = String(now.getHours()).padStart(2, "0");
  const mm = String(now.getMinutes()).padStart(2, "0");
  // Sanitize for filesystem (just in case user-typed ID has weird chars)
  const txnIdForFilename = transactionId.replace(/[^0-9-]/g, "").slice(0, 40);
  const filename = `${endDate}T${hh}-${mm} Transaction #${txnIdForFilename}.pdf`;
  doc.save(filename);
}