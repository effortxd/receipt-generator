...
export async function generateMetaReceipt({
  ...
  // to <invoice DD Mon YYYY>, <invoice HH:MM>" — i.e. it inherits the invoice end timestamp,
  // not a hardcoded 23:59.
  const invoiceEndTime = new Date().toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false });
  const dateRangeText = `From ${fmtMetaDate(startDate)}, 00:00 to ${fmtMetaDate(endDate)}, ${invoiceEndTime}`;
