...
export async function generateMetaReceipt({
  ...
  //   2026-04-01T16-53_Transaction__26486742894346480-26465751236445638.pdf
  // Components: <endDate ISO>T<HH-mm>_Transaction__<transactionId>.pdf
  // Use generation-time clock for HH-mm (matches Meta's behavior of stamping
  // the moment the receipt was issued).
  const now = new Date();
  const hh = String(now.getHours()).padStart(2, "0");
  const mm = String(now.getMinutes()).padStart(2, "0");
  const txnIdForFilename = (config.receiptTransactionId || `${Date.now()}${Math.floor(Math.random() * 1e16)}`)
    .replace(/[^0-9-]/g, "") // keep only digits and hyphens (filesystem-safe)
    .slice(0, 40);
  const filename = `${endDate}T${hh}-${mm}_Transaction__${txnIdForFilename}.pdf`;
  doc.save(filename);
}